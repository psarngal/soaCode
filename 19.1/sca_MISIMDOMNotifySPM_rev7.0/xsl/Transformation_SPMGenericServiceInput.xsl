      ...
      <msg_in_out:PAYLOAD>
        <msg_in_out:SPM>
          <msg_in_out:Subscription>
            ...
            <xsl:if test="/ns1:MISIMD_SPM_SUBSCRIPTION/ns1:SUPERSEDED_PROJECT !='0' and string(/ns1:MISIMD_SPM_SUBSCRIPTION/ns1:SUPERSEDED_PROJECT)">
            ...
            <xsl:for-each select="/ns1:MISIMD_SPM_SUBSCRIPTION/ns1:LINES/ns1:MISIMD_SPM_LINES[ns1:CONTRACT_TYPE=../../ns1:CONTRACT_TYPE]">
              <msg_in_out:SubscribedService>
                ...
                <xsl:if test="string(ns1:CONFIG_ID)">
                  <msg_in_out:ConfigId>
                    <xsl:value-of select="ns1:CONFIG_ID"/>
                  </msg_in_out:ConfigId>
                </xsl:if>
                <xsl:if test="string(ns1:ENFORCED_RATECARD)">
                  <msg_in_out:EnforcedRateCardPricingModel>
                    <xsl:value-of select="ns1:ENFORCED_RATECARD"/>
                  </msg_in_out:EnforcedRateCardPricingModel>
                </xsl:if>
                <xsl:if test="string(ns1:IS_VARIABLE_COMMITMENT)">
                  <msg_in_out:IsVariableCommitment>
                    <xsl:value-of select="ns1:IS_VARIABLE_COMMITMENT"/>
                  </msg_in_out:IsVariableCommitment>
                </xsl:if>
                <xsl:choose>
                  <xsl:when test='ns1:IS_SINGLE_RATECARD = "Yes"'>
                    <msg_in_out:IsSingleRateCard>
                      <xsl:text disable-output-escaping="no">true</xsl:text>
                    </msg_in_out:IsSingleRateCard>
                  </xsl:when>
                  <xsl:otherwise>
                ...
                <xsl:if test="string(ns1:AGREEMENT_TYPE_CODE)">
                  <msg_in_out:AgreementType>
                    <xsl:value-of select="ns1:AGREEMENT_TYPE_CODE"/>
                  </msg_in_out:AgreementType>
                </xsl:if>
