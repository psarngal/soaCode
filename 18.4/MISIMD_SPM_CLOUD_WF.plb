...
-- $Header: MISIMD_SPM_CLOUD_WF.plb 120.117 2018/04/06 11:43:47 nitesaxe noship $
...
--   120.118  2018/04/14  nitesaxe - CF 18.4 + Change to restrict entitlements based on rate card (if populated)
-- ===========================================================================
...
BEGIN
  ...
  BEGIN
	select  count(additional_column46)
	into SPM_LINE_COUNT
	from MISONT_ORDER_LINE_ATTRIBS_EXT
	where header_id = p_header_id
	and additional_column46 = 'SPM';
	EXCEPTION
	WHEN OTHERS THEN
	SPM_LINE_COUNT   := 0;
  END;
...
/* ------------------------------------------SPM-7825---START---------------psarngal-----------------*/
 ...
 SELECT CAST (collect (OSS_INTF_USER.MISIMD_SPM_RATE_CARD (
  ...
  SELECT
    ...
    AND (EXISTS
      (SELECT 1
      FROM misqp_cloud_credits_lines_all
      WHERE rate_card_id  = la.rate_card_id
      AND inv_part_number = MISIMD_SPM_CLOUD_WF.get_service_part(TO_CHAR(msi.segment1))
      )
