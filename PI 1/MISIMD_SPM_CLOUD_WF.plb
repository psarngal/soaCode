...
-- $Header: MISIMD_SPM_CLOUD_WF.plb 120.155 2019/02/08 08:54:00 psarngal noship $
...
--   120.153  2018/12/22  psarngal - CF 18.22
--   120.153  2019/01/08  psarngal - CF 19.01
--   120.155  2019/03/23  psarngal - CF 19.03
-- ===========================================================================
...
END;
  ...
  SELECT oh.header_id,
    ...
    oh.order_number,
    (TO_CHAR(oh.ordered_date, 'YYYY-MM-DD HH24:MI:SS')) ORDERED_DATE,
   ...
   -- END BUG 27476500
    ...
    and ( nvl(pricing_attribute94,'X') <> 'CONVERT_TO_UCM' or nvl(pricing_attribute94,'X') <> 'RENEW_TO_UCM') -- rdnagara Enh 29321802
    and rownum=1),
...
-- rdnagara END
  ...
  (SELECT line_id, (SELECT pricing_attribute94 from oe_order_price_attribs where line_id = ol.line_id) operation_type
    ...
    AND flow_status_code           NOT IN ( 'CANCELLED','CLOSED' ) --NPI-416
  ...
  AND application = 'MISIMD_SPM_CLOUD_WF'
    ...
    AND ol.header_id                = p_header_id
    AND ol.flow_status_code        NOT IN ('CANCELLED','CLOSED') --NPI-416
    AND is_spm_eligible(ol.line_id) ='Y'
    AND EXISTS
      (SELECT 1
      FROM oe_order_price_attribs
  ...
  opa.pricing_attribute51 SUPERSEDE_NOTES,
   ...
   --Changes for SPM-11135 begin- SPMPROV
    ...
    (SELECT ADDITIONAL_COLUMN48 FROM misont_order_line_attribs_ext WHERE line_id = ol.line_id) IS_VARIABLE_COMMITMENT,  --SPM-16168
    (SELECT CUST_PO_NUMBER   FROM oe_order_lines_all WHERE line_id = ol.line_id) PO_NUMBER,                             --SPM-14743
    (SELECT ADDITIONAL_COLUMN71  FROM misont_order_line_attribs_ext WHERE line_id = ol.line_id) IS_SINGLE_RATECARD,      --SPM-19388
    (SELECT ADDITIONAL_COLUMN80  FROM misont_order_line_attribs_ext WHERE line_id = ol.line_id) CUST_TRANS_REF_NUMBER    --SPM-20823
   ...
   --Changes for SPM-9570 - SPMPROV
    ...
    v_payload.LINES(v_payload.LINES.count).IS_VARIABLE_COMMITMENT,        --SPM-16168
    v_payload.LINES(v_payload.LINES.count).PO_NUMBER,                     --SPM-14743
    v_payload.LINES(v_payload.LINES.count).IS_SINGLE_RATECARD             --SPM-19388
    v_payload.LINES(v_payload.LINES.count).CUST_TRNX_REF_NUM              --SPM-20823
  FROM oe_order_lines_all ol,
    oe_order_price_attribs opa,
  ...
  SELECT oe.name,ooha.agreement_id,TO_CHAR(TO_DATE(oe.end_date_active ),'YYYY-MM-DD HH24:MI:SS'),oe.AGREEMENT_TYPE_CODE
  INTO v_payload.LINES(v_payload.LINES.count).AGREEMENT_NAME,--SPM-13242
       ...
       v_payload.LINES(v_payload.LINES.count).AGREEMENT_TYPE_CODE --SPM-17765
...
/* ------------------------------------------SPM-7825---START---------------psarngal-----------------*/
  BEGIN
    ...
    --
    /*ELSIF (v_payload.LINES(v_payload.LINES.count).OPERATION_TYPE = 'ONBOARDING' OR v_payload.LINES(v_payload.LINES.count).OPERATION_TYPE = 'RAMPED_ONBOARDING' OR v_payload.LINES(v_payload.LINES.count).OPERATION_TYPE = 'RAMPED_UPDATE' )THEN
        BEGIN
            v_payload.LINES(v_payload.LINES.count).CONTRACT_TYPE := 'Cloud Universal Subscriptions';
...
--
FUNCTION contains_metered_subscription (
    p_header_id IN NUMBER)
  RETURN BOOLEAN
AS
  l_is_metered_billing VARCHAR2 (100) := 'N';
...
--Bug 25341009 HIGH IO: dyzg9hzqtcpsx, 5w1cz6kx54c5v - APPS.MISIMD_SPM_CLOUD_WF - rdnagara
            ...
            FROM
                (
                    SELECT /*+ NO_MERGE leading(y) */
                        category_set_id,
                        category_id
                    FROM
                        mtl_item_categories mic,
                        (
                            SELECT /*+ leading(x) use_nl(oel) */ DISTINCT
                                inventory_item_id
                            FROM
                                oe_order_lines_all oel,
                                (
                            ...
                            WHERE
                                oel.line_id = x.line_id
                        ) y
                    WHERE
                        mic.organization_id = 14354
                        AND mic.inventory_item_id = y.inventory_item_id
                ) z,
                mtl_categories mc,
                mtl_category_sets mcs
            WHERE
                mcs.category_set_id = z.category_set_id
                AND mc.category_id = z.category_id
                AND lower(mcs.category_set_name) LIKE 'usage%billing%'
                AND mc.segment1 = 'METERED_BILLING'
                AND ROWNUM = 1;
        IF (l_is_metered_billing = 'Y') THEN
            RETURN TRUE;
        ELSE 
           NULL;
        END IF;
...
WHEN OTHERS THEN
  RETURN FALSE;
END contains_metered_subscription;
--
