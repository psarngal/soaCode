...
-- $Header: MISIMD_SPM_CLOUD_WF.plb 120.109 2018/03/02 07:59:42 rkasinad noship $
...
AND pricing_attribute93 ='PROVISIONED';
 ...
 WHERE domain = 'MISIMD_SPM_CLOUD_BRIDGE'
 AND code IN ('START_END_DATES');
    EXCEPTION
        WHEN OTHERS THEN
  ...
  SELECT oh.header_id,
    ...
    WHERE header_id = p_header_id  AND pricing_attribute94 IS NOT NULL AND ROWNUM=1),p_operation_type) OPERATION_TYPE,
     ...
     (select TO_CHAR(min(oola.service_start_date), 'YYYY-MM-DD')||' '
      ...
      where is_spm_eligible(oola.line_id) = 'Y' and oola.header_id = oopa.header_id and oopa.pricing_attribute93 = 'PROVISIONED' and oola.header_id = oh.header_id)
      ELSE
...
IF(p_operation_type <> 'ONBOARDING' AND v_extension_update_flag = 'Y')
 THEN
	...
	oep.pricing_attribute94 IS NOT NULL AND oep.pricing_attribute94='EXTENSION'
...
-- rdnagara END
  ...
  opa.pricing_attribute51 SUPERSEDE_NOTES,
   ...
   --Changes for SPM-11135 begin- SPMPROV
    ...
    (SELECT oe.name FROM oe_order_headers_all ooha,
        oe_agreements_v oe
        WHERE ooha.header_id = p_header_id
        AND ooha.agreement_id = oe.agreement_id
        AND EXISTS
        ...
        WHERE transaction_type_id = ooha.order_type_id
        AND name LIKE '%CLOUD%'
        AND language = 'US')) AGREEMENT_NAME,								       					                              --SPM-13242
  ...
  INTO v_payload.LINES(v_payload.LINES.count).ORDER_LINE_ID,
   ...
   --Changes for SPM-9570 - SPMPROV
    ...
    v_payload.LINES(v_payload.LINES.count).AGREEMENT_NAME,				      --SPM-13242
