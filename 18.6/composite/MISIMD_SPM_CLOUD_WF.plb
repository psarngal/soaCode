...
-- $Header: MISIMD_SPM_CLOUD_WF.plb 120.127 2018/05/17 09:23:35 psarngal noship $
...
--   120.127  2018/06/23  psarngal - CF 18.6
-- ===========================================================================
...
END;
  ...
  SELECT oh.header_id,
   ...
   -- END BUG 27476500
    ...
    and ( pricing_attribute94 <> 'CONVERT_TO_COMMIT' or pricing_attribute94 <> 'RENEW_TO_COMMIT')
    and rownum=1),
...
/* ------------------------------------------SPM-7825---START---------------psarngal-----------------*/
  BEGIN
    IF  ((v_order_commit_model IS NOT NULL) OR ((v_payload.LINES(v_payload.LINES.count).OPERATION_TYPE ='CONVERT_TO_COMMIT' or v_payload.LINES(v_payload.LINES.count).OPERATION_TYPE = 'RENEW_TO_COMMIT'))) THEN
    --
      BEGIN
          BEGIN
            SELECT lookup_value
