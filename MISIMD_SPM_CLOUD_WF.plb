...
-- $Header: MISIMD_SPM_CLOUD_WF.plb 120.103 2017/12/18 05:21:38 psarngal noship $
...
--   120.105  2018/02/03  psarngal - CF 18.2
-- ===========================================================================
...
IS
  ...
  IS
    ...
    FROM
      (SELECT pricing_attribute94 OPERATION_TYPE,
        MIN(pricing_attribute58) SEQUENCE_ID
      FROM oe_order_price_attribs
      WHERE header_id          = p_header_id
      AND pricing_attribute92  = p_subscription_id
      AND pricing_attribute94 IS NOT NULL
      GROUP BY pricing_attribute94
 ...
 EXCEPTION WHEN OTHERS THEN
  ...
  --Get Automated Flow Flag
   BEGIN
    ...
    FROM OSS_INTF_USER.MISIMD_SPM_LOOKUPS
    WHERE lookup_type = 'SPM_PROV_FLOW'
  ...
  ELSE
  UPDATE misont_order_line_attribs_ext
  SET SPM_INTERFACE_ERROR = 'SPM_PROV_FLOW:AUTOMATED_FLOW is not Enabled - Retry Manually after setting the flag to Y in OSS_INTF_USER.MISIMD_SPM_LOOKUPS'
  WHERE header_id         = p_header_id;
  END IF;
  END IF;
EXCEPTION
...
AS
  ...
  AND sc.sales_credit_type_id IN (select sales_credit_type_id from ONT.oe_sales_credit_types where quota_flag = 'Y')
  AND sc.percent        > 0
...
BEGIN
  ...
  SELECT oh.header_id,
    ...
    (select TO_CHAR(min(service_start_date), 'YYYY-MM-DD')||' '
      ...
      where is_spm_eligible(line_id) = 'Y' and header_id = oh.header_id) START_DATE,
    (select TO_CHAR(max(service_end_date), 'YYYY-MM-DD')||' '
      ||TO_CHAR(max(service_end_date), 'HH24:MI:SS') from oe_order_lines_all
      where is_spm_eligible(line_id) = 'Y' and header_id = oh.header_id) END_DATE,
    (SELECT months_between(MAX(service_end_date+1),MIN(service_start_date)) FROM oe_order_lines_all
      where is_spm_eligible(line_id) = 'Y' and header_id = oh.header_id) DURATION,
  ...
  BEGIN
    ...
    FROM OSS_INTF_USER.MISIMD_SPM_LOOKUPS
    WHERE lookup_type = 'INVOICE_SCHEDULE'
  ...
  -- CHECK OPCM ORDER--
  --
  SELECT COUNT(pricing_attribute99) into  v_count_extsite_lines  FROM oe_order_price_attribs
  WHERE LINE_ID IN (SELECT LINE_ID  FROM MISONT_ORDER_LINE_ATTRIBS_EXT  WHERE HEADER_ID =p_header_id)
  AND pricing_attribute99 ='EXTSITE';
  --
  --
  SELECT COUNT(*) INTO v_count_textura_lines FROM (SELECT misont_cloud_pub2.get_cloud_servicegroup_tag(ol.ordered_item,'SERVICE_GROUP') AS service_group
FROM oe_order_lines_all ol
WHERE ol.header_id    = p_header_id
AND ol.item_type_code = 'SERVICE') WHERE SERVICE_GROUP IN ('TEXTURACPMGCMB','TEXTURATPMGCMB','TEXTURATPMSCMB','TEXTURACPMSCMB');
  ...
  (SELECT line_id, (SELECT pricing_attribute94 from oe_order_price_attribs where line_id = ol.line_id) operation_type
   ...
   UNION /*-- replaced UNION ALL with UNION for Bug 26314070 fix*/
    ...
    (SELECT meaning
    FROM misimd_spm_lookups
    WHERE lookup_code = 'ONE_TIME_ITEM_CATEGORY'
    AND enabled_flag  = 'Y'
    )
  ...
  SELECT ol.line_id,
    ...
    'OBSCNTR_'||substr(service_period,1,1) DURATION_UNIT,
	  DECODE ((SELECT UPPER(ADDITIONAL_COLUMN28) FROM misont_order_line_attribs_ext
    WHERE line_id = ol.line_id),'TENCENT' ,
  ...
  opa.pricing_attribute51 SUPERSEDE_NOTES,
   ...
   (SELECT ADDITIONAL_COLUMN15 FROM misont_order_line_attribs_ext WHERE line_id = ol.line_id) CLOUD_ACCOUNT_ID,
   (SELECT ADDITIONAL_COLUMN14 FROM misont_order_line_attribs_ext WHERE line_id = ol.line_id) CLOUD_ACCOUNT_NAME,
   ...
   --Changes for SPM-11135 begin- SPMPROV
    ...
    (SELECT ooha.agreement_id FROM oe_order_headers_all ooha,
        oe_agreements_v oe
        ...
        AND name LIKE '%CLOUD%'
        AND language = 'US')) AGREEMENT_ID,    --SPM-12871
  ...
  INTO v_payload.LINES(v_payload.LINES.count).ORDER_LINE_ID,
   ...
   --Changes for SPM-9570 - SPMPROV
    ...
    v_payload.LINES(v_payload.LINES.count).AGREEMENT_ID,				--SPM-12871
...
/* ------------------------------------------SPM-7825---START---------------psarngal-----------------*/
  BEGIN
    IF  v_order_commit_model IS NOT NULL THEN
    --
      BEGIN
          BEGIN
            SELECT MEANING
      ...
      --
          ...
          BEGIN
              ...
              SELECT MEANING INTO v_contract_type
              FROM OSS_INTF_USER.MISIMD_SPM_LOOKUPS
              WHERE LOOKUP_TYPE ='PRICING_MODEL'
 ...
 /* ---------------------------------------------SPM-7825---END---------------psarngal-----------------*/
  ...
  --
    ...
    AND NVL(hcar.role_type,'CONTACT')   = 'CONTACT'
    AND hcar.party_id                   = hr.party_id(+)
    AND NVL(hr.relationship_code,'CONTACT_OF')   = 'CONTACT_OF'
    AND hr.subject_id                   = hpc.party_id(+)
    AND NVL(hr.subject_type,'PERSON')   = 'PERSON';
  EXCEPTION WHEN OTHERS THEN
  NULL;
 ...
 SELECT CAST (collect (OSS_INTF_USER.MISIMD_SPM_RATE_CARD (
  ...
  SELECT
    CAST(multiset
    (SELECT OSS_INTF_USER.MISIMD_SPM_ENT_COMPONENT(msi2.inventory_item_id
      ...
      , to_number(bom_component.item_num)+10
      -- IS LINE_NUMBER
      , mc.segment1
      -- IS LICENSE_METRIC
      , '0'
    ...
    AND msi2.inventory_item_id                       = oel.inventory_item_id
...
-- rdnagara
	 insert_log (p_module =>'Set_SPM_info',
      ...
      p_context_id1 => p_line_id);
        ...
        and    SPM_INTERFACE_STATUS <> 'SPM_INTERFACED'
